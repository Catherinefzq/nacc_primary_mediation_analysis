...
mediate_multi <- function(model.m, model.y, sims = 1000, 
                          seed = 1234,
    ...
    if (!is.null(weights.m) &&
        isMer.m &&
        class(model.m)[[1]] == "glmerMod" && FamilyM == "binomial") {
        message("weights taken as sampling weights, not total number of trials")
    }
    if (is.null(weights.m)) {
    ...
    se.ranef.new <- function (object) {
        ...
        for (m in 1:n.groupings) {
            ...
            dimnames(se.bygroup[[m]]) <-
                list(names.full[[1]], names.full[[2]])
        }
        return(se.bygroup)
    }
    
    ## need to set seed everytime ###
    set.seed(seed+1)
    ...
    for (e in 1:4) {
        ...
        for (j in 1:sims) {
            ...
            # Set mediator values
            PredictMt <-
                PredictM1[j, ] * tt[3] + PredictM0[j, ] * (1 - tt[3])
            PredictMc <-
                PredictM1[j, ] * tt[4] + PredictM0[j, ] * (1 - tt[4])
        ...
        effects.tmp[, , e] <-
            Pr1 - Pr0 ### e=1:mediation(1); e=2:mediation(0); e=3:direct(1); e=4:direct(0)
        rm(Pr1, Pr0)
    }
...
################################################################################
integrate_multimed <- function(..., conf.level = 0.95){
    models <- list(...)
    if (unique(unlist(lapply(models, class))) != "multimed"){
        stop("input models are not multimed object")
    }
    else{
        model.list <- list()
        for (i in 1:length(models)) {
            # extract the simulation draws of effects
            model.list[[i]] <-
                models[[i]][c("d0.sims","d1.sims", # mediation effect
                              "z0.sims","z1.sims", # direct effect
                              "n0.sims", "n1.sims", # proportion mediated
                              "tau.sims", "mediator")] # total effects
        }
    }
    # set confidence interval
    low <- (1 - conf.level)/2
    high <- 1 - low
    # p value compute function (extract from mediation::mediate)
    pval <- function(x, xhat){
        if (xhat == 0) out <- 1
        else {
            out <- 2 * min(sum(x > 0), sum(x < 0)) / length(x)
    ...
    ## aggregate direct effect ##
    n.model <- length(model.list)
    sum.zeta <- purrr::map2(model.list, 1:n.model, ~ .x$z0.sims + .x$z1.sims) %>% 
        purrr::transpose() %>% 
        lapply(., function(x){sum(unlist(x))}) %>% 
        unlist() 
    sum.zeta.avg <- sum.zeta/(2*n.model)
    sum.z.avg <- mean(sum.zeta.avg)
    ci.sum.z.avg <- quantile(sum.zeta.avg, c(low,high), na.rm = TRUE)
    p.sum.z.avg <- pval(sum.zeta.avg, sum.z.avg)
    
    ## aggregate total effect ##
    sum.tau <- purrr::transpose(model.list)[["tau.sims"]] %>% 
        purrr::transpose() %>% 
        lapply(., function(x){sum(unlist(x))}) %>% 
        unlist()
    sum.tau <- sum.tau - 2*sum.zeta.avg
    sum.tau.coef <- mean(sum.tau)
    ci.sum.tau <- quantile(sum.tau, c(low,high), na.rm = TRUE)
    p.sum.tau <- pval(sum.tau, sum.tau.coef)
    
    ## aggregate prop mediated ##
    all.nu.avg <- purrr::map2(model.list, 1:n.model, ~ .x$d0.sims + .x$d1.sims) %>% 
        lapply(., function(x){x/(2*sum.tau)})
    all.n.avg <- purrr::map2(model.list, 1:n.model,
                             ~ mean(.x$d0.sims) + mean(.x$d1.sims)) %>% 
        lapply(., function(x){x/(2*sum.tau.coef)})
    ci.all.n.avg <- lapply(all.nu.avg, function(x){
        quantile(x, c(low,high), na.rm = TRUE)
    })
    p.all.n.avg <- mapply(function(x, y){
        pval(x, y)
    },
    x = all.nu.avg, y = all.n.avg)
    
    sum.nu <- purrr::map2(model.list, 1:n.model, ~ .x$d1.sims + .x$d0.sims) %>% 
        purrr::transpose() %>% 
        lapply(., function(x){sum(unlist(x))}) %>% 
        unlist() 
    sum.nu.avg <- sum.nu/(2*sum.tau)
    sum.n.avg <-  purrr::map2(model.list, 1:n.model,
                              ~ mean(.x$d0.sims) + mean(.x$d1.sims)) %>% 
        unlist() %>% 
        sum()
    sum.n.avg <- sum.n.avg/(2*sum.tau.coef)
    ci.sum.n.avg <- quantile(sum.nu.avg, c(low,high), na.rm = TRUE)
    p.sum.n.avg <- pval(sum.nu.avg, sum.n.avg)
    
    ## aggregate ACME ##
    all.delta.avg <- purrr::map2(model.list, 1:n.model, ~ .x$d0.sims + .x$d1.sims) %>% 
        lapply(., function(x){x/2})
    all.d.avg <- lapply(all.delta.avg, mean)
    ci.all.delta.avg <- lapply(all.delta.avg, function(x){
        quantile(x, c(low, high), na.rm = TRUE)
    })
    p.all.delta.avg <- mapply(function(x, y){
        pval(x, y)
    },
    x = all.delta.avg, y = all.d.avg)
    
    ## generate the table
    sumtb <- 
        tibble(names = c("Direct Effect", "Total Effect", 
                         paste("Prop.Mediate",
                               unlist(transpose(model.list)["mediator"]), sep = "_"), 
                         "Prop.Mediated_all",
                         paste("ACME",
                               unlist(transpose(model.list)["mediator"]), sep = "_")),
               Estimate = c(sum.z.avg, sum.tau.coef, unlist(all.n.avg),
                            sum.n.avg, unlist(all.d.avg)),
               ci_lower = c(ci.sum.z.avg[1], ci.sum.tau[1], 
                            unlist(lapply(ci.all.n.avg, `[[`, 1)),
                            ci.sum.n.avg[1], 
                            unlist(lapply(ci.all.delta.avg, `[[`, 1))),
               ci_upper = c(ci.sum.z.avg[2], ci.sum.tau[2], 
                            unlist(lapply(ci.all.n.avg, `[[`, 2)),
                            ci.sum.n.avg[2], 
                            unlist(lapply(ci.all.delta.avg, `[[`, 2))),
               p_value = c(p.sum.z.avg, p.sum.tau, 
                           unlist(p.all.n.avg), 
                           p.sum.n.avg, 
                           unlist(p.all.delta.avg)))
    return(sumtb)
}

#################################################################################
mediate.join.glm <- function(model.m, model.y, sims = 10, 
                             boot = FALSE, boot.ci.type = "perc",
                             treat = "treat.name", mediator = "med.name",
                             conf.level = .95, robustSE = TRUE, dropobs = FALSE,
                             seed = 1234,...){
    # default some settings in original function
    # only support GLM/LM mediator model and outcome model
    covariates = NULL; outcome = NULL;
    control = NULL; control.value = 0; treat.value = 1;
    long = TRUE; cluster = NULL; group.out = NULL; use_speed = FALSE
    ...
    # unsupported type in current adjusted function
    if(isGam.y|isGam.m|isRq.y|isRq.m|isSurvreg.y|isSurvreg.m|isMer.y|isMer.m|isOrdered.y){
        stop("Current type of model.m or model.y not supported. Use one of glm, lm, polr")
    }
    ...
    # Specify group names (NO GLMER so no group names)
    group.m <- NULL
    group.y <- NULL
    group.out <- NULL
